    ...
    * Note * If Views have been created that reference the Service, this function will also touch the View items to update
    ...
    def touchItem(item, message, outcome):
        ...
        try:
            ...
            if verbose:
                print(
                    " * Failed to Touch details for Item Id: '{}', Outcome: '{}'".format(
                        item.id, e
                    )
                )
            ...
            status = (
                status
                if not "error code" in status.lower()
                else status.replace("\n", " ")
            )
    ...
    if not item.type == "Feature Service":
        outcome["items"].append(
            {
                ...
                "itemType": item.type,
                "action": "verify",
                "result": "Item Type is NOT a 'Feature Service'",
            }
        )
            ...
            outcome["items"].append(
                {
                    ...
                    "itemType": item.type,
                    "action": "verify",
                    "result": "Missing Associated Service Data Item or datafile Item 'name'",
                }
            )
            outcome["success"] = False  # Set as error
        ...
        if updateFile:
            ...
            serviceLastModified = (
                0
                if not layers
                else layers[0].get("editingInfo", {}).get("lastEditDate", 0)
            )
            serviceLastModified = (
                0
                if not serviceLastModified
                else datetime.datetime.utcfromtimestamp(int(serviceLastModified / 1000))
            )

            if verbose and serviceLastModified:
                print(
                    " - Service Last Modified: {}".format(
                        serviceLastModified.strftime("%a, %d %b %Y %H:%M:%S GMT")
                    )
                )
            ...
            if updateFile.split(":")[0].lower() in ["ftp", "http", "https"]:
                ...
                fileLastModified = (
                    ...
                    if not os.path.exists(outputFile)
                        ...
                        int(os.stat(outputFile).st_mtime)
                    )
                )

                # Set lastModified to file lastModified if Service details are not available!
                lastModified = (
                    serviceLastModified if serviceLastModified else fileLastModified
                )
                ...
                try:
                    ...
                    authHandlers.append(
                        urllib.request.HTTPSHandler(
                            context=urllib.request.ssl.SSLContext(
                                urllib.request.ssl.PROTOCOL_SSLv23
                            )
                        )
                    )
                    ...
                    urllib.request.install_opener(
                        urllib.request.build_opener(*authHandlers)
                    )

                    request = urllib.request.urlopen(updateFile)
                    headers = (
                        dict(request.info()._headers)
                        if hasattr(request, "info")
                        else {}
                    )  # Get Header 'Tuple' list and convert to dictionary

                    if verbose:
                        print(
                            " - Download to: '{}', Last Modified: {}".format(
                                outputFile,
                                fileLastModified.strftime("%a, %d %b %Y %H:%M:%S GMT")
                                if fileLastModified
                                else "N/A",
                            )
                        )
                    ...
                    try:
                        ...
                        if verbose:
                            print(
                                " * Issue Ignored * But, unable to compare 'Last-Modified' Dates, Error: '{}'".format(
                                    e
                                )
                            )
                    ...
                    if verbose:
                        print("\nDownloading Data...")
                    updateFile, headers = urllib.request.urlretrieve(
                ...
                outcome["items"].append(
                    {
                        ...
                        "itemType": item.type,
                        "action": "verify",
                        "result": "Update Filename '{}' does NOT match Original Filename used to Publish Service: '{}'".format(
                            updateFile, outputFile
                        ),
                    }
                )
            ...
            try:
                ...
                if verbose:
                    print(
                        " * Failed to Update Item, Id: '{}', Outcome: '{}'".format(
                            item.id, e
                        )
                    )
                status = "Failed, Outcome: '{}'".format(e)
    ...
    if touchItems and outcome["success"] == True:
        for view in item.related_items("Service2Service"):
            touchItem(
                view,
                "\nTouching details for related View: '{}'".format(view.title),
                outcome,
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "\n{} Usage: Python {} <profile> <item> <title> [<filename>[, <url>]]".format(
                version, __file__
            )
        )
        print(
        ...
        print(
            "     <title>: (required) Title of Item, to verify id matches correct item."
        )
        print(
            "  <filename>: (optional) File path and/or name, or URL, to overwrite Service data with."
        )
        print(
            "                         Default: None, only touch Item and Views to refresh last update date."
        )

        exit("\n\a * ERROR * Insufficient Input Parameters, please review 'Usage'!")

    profile, itemId, itemTitle = sys.argv[1:4]
    updateFile, updateExists = (
        (None, False)
        if len(sys.argv) < 5
        else (
            sys.argv[4],
            os.path.exists(sys.argv[4])
            if not sys.argv[4].split(":")[0].lower() in ["ftp", "http", "https"]
            else None,
        )
    )
    fileLastModified = (
        0
        if not updateExists
        else datetime.datetime.utcfromtimestamp(int(os.stat(updateFile).st_mtime))
    )
    usingPro = profile.lower() == "pro"

    print("\n           Running: {}, {}".format(__file__, version))
    print(
        "Python API Profile: {}{}".format(
            profile, " (using active ArcGIS Pro connection)" if usingPro else ""
        )
    )
        ...
        "   Upload Filename: {}{}".format(
            *(
                ...
                if updateExists
                    ...
                    (
                        (updateFile, " (NOT Found!)")
                        if updateExists is False
                        else (updateFile, " (From URL!)")
                    )
                    if updateFile and updateExists is None
                    else ("None, Touch item & views only!", "")
                )
            )
        )
    )
    ...
    if not usingPro:
        ...
        if hasattr(arcgis.GIS, "_securely_get_password"):
            ...
            getPassword = (
                arcgis.gis._impl._profile.ProfileManager()._securely_get_password
            )
        else:
            print(
                " - Cannot check password for Python API v{}, unable to securely get password!".format(
                    arcgis.__version__
                )
            )
        ...
        if getPassword:
            ...
            if os.path.isfile(gis_cfg_file_path) and profile:  # v1.5.0
                ...
                username = (
                    gisConfig[profile]["username"]
                    if gisConfig.has_option(profile, "username")
                    else None
                )
                ...
                if username is not None and username:
                    if getPassword(profile) is None:
                        exit(
                            "\n\a * ERROR * Password missing for user '{}' in Profile '{}'!".format(
                                username, profile
                            )
                        )
        ...
        print(
            " - Logged in with user: '{} ({})', On: '{}'".format(
                gis.users.me.username, gis.users.me.fullName, gis.url
            )
        )
    ...
    if not item.title == itemTitle:
        exit(
            "\a * ERROR * Feature Service Item Title does NOT match specified Title: '{}', Found: '{}'".format(
